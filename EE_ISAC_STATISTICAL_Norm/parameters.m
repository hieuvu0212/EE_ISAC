...
%  Channel model:  h_k = h_hat_k + Delta_h_k
%    Delta_h_k ~ CN(0, xi_k^2 * I_N)
%    xi_k^2 = delta^2 * || vec(h_hat_k) ||^2   (per-user, data-dependent)
%    delta      % Relative channel error magnitude (same for all users)
%    delta_outage : Outage probability (Pr{ SINR_k < gamma_min } <= delta_outage) 
% -------------------------------------------------------------------------
function p = parameters()
    ...
    p.delta  = 0.01;          % Relative channel error magnitude (same for all users)

    % Uncertainty xi_k^2 = delta^2 * || vec(h_hat_k) ||^2   (per-user, data-dependent)
